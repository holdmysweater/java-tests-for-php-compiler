...
public final class PhpRuntime {
    ...
    public static PhpClass requireClass(String name) {
        ...
        // If not registered yet, try to force JVM class load so <clinit> runs
        // and generated bytecode can call defineClass(...).
        if (c == null && !key.isEmpty()) {
            jvmClassFromPhpName(key);   // uses Class.forName(norm(...)) :contentReference[oaicite:4]{index=4}
            c = CLASSES.get(key);
        }
